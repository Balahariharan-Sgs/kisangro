...
class _kycState extends State<kyc> {
  ...
  Future<Uint8List?> _loadImageFromUrl(String imageUrl) async {
    ...
    try {
      ...
      debugPrint('KYC Screen: Response status for $completeUrl: ${response.statusCode}');
      debugPrint('KYC Screen: Content-Type: ${response.headers['content-type']}');
      debugPrint('KYC Screen: Content-Length: ${response.headers['content-length']}');

      if (response.statusCode == 200) {
        if (_isValidImageData(response.bodyBytes)) {
          debugPrint('KYC Screen: ✅ Image loaded successfully from: $completeUrl');
          debugPrint('KYC Screen: Image size: ${response.bodyBytes.lengthInBytes} bytes');
          return response.bodyBytes;
        } else {
          debugPrint('KYC Screen: ❌ Response is not a valid image: $completeUrl');
        }
      } else {
        debugPrint('KYC Screen: ❌ HTTP ${response.statusCode} for: $completeUrl');
      }
  ...
  Future<void> _fetchKycData(int cusId) async {
    ...
    debugPrint('KYC Screen: Attempting to fetch existing KYC data for cus_id: $cusId');
    try {
      // First try to fetch using the new API (type: 1024)
      ...
      if (response['error'] == false && response['data'] != null) {
        ...
        final kycData = {
          ...
          'nature_of_business': 'Distributor', // Default as not provided in new API
        ...
        if (kycData['photo']!.isNotEmpty) {
          ...
          if (imageBytes == null) {
            debugPrint('KYC Screen: Failed to load image, will show placeholder');
          }
        }
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          setState(() {
            ...
            _businessContactNumberController.text = kycData['business_contact_number']!;
        ...
        debugPrint('KYC Screen: Successfully fetched and populated data from new API');
      } else {
      ...
      debugPrint('KYC Screen: Error fetching data from new API: $e. Trying old API...');
      await _fetchKycDataOldApi(cusId);
    } finally {
      setState(() {
    ...
    try {
      ...
      if (response['status'] == 'success') {
        ...
        final kycData = {
          ...
          'business_contact_number': responseData['business_contact_number'] ?? '',
          'isGstinVerified': responseData['isGstinVerified'] ?? false,
          'photo': responseData['photo'] ?? '', // Get photo URL from old API too
        };

        // Load image from URL if available
        Uint8List? imageBytes;
        if (kycData['photo']!.isNotEmpty) {
          debugPrint('KYC Screen: Photo URL found in old API: ${kycData['photo']}');
          imageBytes = await _loadImageFromUrl(kycData['photo']!);
        }
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          setState(() {
            ...
            if (imageBytes != null) {
              _imageBytes = imageBytes;
              debugPrint('KYC Screen: Image loaded and set successfully from old API');
            }
          });
        ...
        debugPrint('KYC Screen: Successfully fetched and populated data from old API');
      } else {
        debugPrint('KYC Screen: No data found for cus_id: $cusId or API call failed. Loading local data instead.');
        _loadExistingKycData();
      }
    } catch (e) {
      debugPrint('KYC Screen: Error fetching data from old API: $e. Loading local data instead.');
      _loadExistingKycData();
    }
  }
    ...
    try {
      ...
      if (existingBusinessData != null) {
        setState(() {
          ...
          _gstinDetails = existingBusinessData.gstin != null && existingBusinessData.businessAddress != null
              ? {'GSTIN': existingBusinessData.gstin, 'Business Address': existingBusinessData.businessAddress}
              : null;
          ...
          _hasExistingKycData = existingBusinessData.fullName != null && existingBusinessData.fullName!.isNotEmpty;
  ...
  Future<void> _pickImage(ImageSource source) async {
    ...
    try {
      ...
      if (pickedFile != null) {
        ...
        debugPrint('KYC Screen: Compressed image size: ${bytes.lengthInBytes} bytes (${(bytes.lengthInBytes / 1024).toStringAsFixed(1)} KB)');

        // Only proceed if image is under 100KB
        if (bytes.lengthInBytes > 100000) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Image is still too large (${(bytes.lengthInBytes / 1024).toStringAsFixed(1)} KB). Please try a different image or take a new photo.', style: GoogleFonts.poppins()),
        ...
        debugPrint('KYC Screen: Image successfully set. Size: ${bytes.lengthInBytes} bytes');

        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Image selected successfully! Size: ${(bytes.lengthInBytes / 1024).toStringAsFixed(1)} KB', style: GoogleFonts.poppins()),
  ...
  Widget _sectionTitle(String title, bool isTablet, bool isDarkMode) {
    ...
    return Padding(
      ...
      child: Align(
        ...
        child: Text(
          title,
          style: GoogleFonts.poppins(fontSize: isTablet ? 18 : 16, fontWeight: FontWeight.bold, color: orangeColor),
        ),
      ),
    );
  }
  ...
  Widget _textFormField(
    ...
    final Color fillColor = isDarkMode ? Colors.grey[700]! : const Color(0xfff8bc8c);
    final Color borderColor = isDarkMode ? Colors.grey[600]! : Colors.transparent;
    final Color focusedBorderColor = isDarkMode ? Colors.white : const Color(0xfff8bc8c);
    final Color orangeColor = const Color(0xffEB7720);
      ...
      child: Padding(
        ...
        child: TextFormField(
          ...
          decoration: InputDecoration(
            ...
            hintText: hintText,
            hintStyle: GoogleFonts.poppins(color: hintColor, fontWeight: FontWeight.bold),
              ...
              child: ElevatedButton(
                onPressed: () async {
                  ...
                  if (_gstinController.text.isEmpty) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text('Please enter GSTIN number', style: GoogleFonts.poppins()),
                  ...
                  try {
                    ...
                    if (response['status'] == 'success') {
                      final address = _extractAddressFromResponse(response);
                      setState(() {
                        _isGstinVerified = true;
                        _gstinDetails = {
                          'GSTIN': _gstinController.text,
                          'Business Address': address ?? 'Address verified',
                        };
                      });
                      
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text('GSTIN verified successfully!', style: GoogleFonts.poppins()),
                          backgroundColor: Colors.green,
                        ),
                      );
                    } else {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text('GSTIN verification failed: ${response['message']}', style: GoogleFonts.poppins()),
                          backgroundColor: Colors.red,
                        ),
                      );
                    }
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text('Error verifying GSTIN: $e', style: GoogleFonts.poppins()),
                        backgroundColor: Colors.red,
                      ),
                    );
                  }
                ...
                child: Text('Verify', style: GoogleFonts.poppins(fontSize: isTablet ? 14 : 12, color: Colors.white)),
              ),
            )
                : null,
          ),
          style: GoogleFonts.poppins(fontSize: isTablet ? 16 : 14, color: textColor),
          validator: null,
        ),
      ),
    );
  ...
  Widget _dropdownField(String label, String? selectedValue, ValueChanged<String?> onChanged, bool isTablet, bool isDarkMode) {
    final Color textColor = isDarkMode ? Colors.white : Colors.black;
    final Color fillColor = isDarkMode ? Colors.grey[700]! : const Color(0xfff8bc8c);
    final Color borderColor = isDarkMode ? Colors.grey[600]! : Colors.transparent;

    return Padding(
      padding: EdgeInsets.symmetric(horizontal: isTablet ? 20 : 10, vertical: isTablet ? 8 : 6),
      child: SizedBox(
        height: isTablet ? 80 : 70,
        child: DropdownButtonFormField<String>(
          ...
          decoration: InputDecoration(
            ...
            fillColor: fillColor,
            labelText: label,
            labelStyle: GoogleFonts.poppins(color: isDarkMode ? Colors.grey[400]! : Colors.black),
            border: OutlineInputBorder(borderSide: BorderSide(color: borderColor)),
            enabledBorder: OutlineInputBorder(borderSide: BorderSide(color: borderColor)),
            focusedBorder: OutlineInputBorder(borderSide: BorderSide(color: isDarkMode ? Colors.white : const Color(0xfff8bc8c), width: 2)),
            contentPadding: EdgeInsets.symmetric(vertical: isTablet ? 20 : 16, horizontal: isTablet ? 20 : 12),
          ),
          items: const [
  ...
  Widget _photoUploadBox(bool isTablet, bool isDarkMode) {
    ...
    final Color containerColor = isDarkMode ? Colors.grey[700]! : Colors.grey.shade200;
    final Color borderColor = isDarkMode ? Colors.grey[600]! : Colors.grey;
    final Color iconColor = const Color(0xffEB7720);

    return Padding(
      padding: EdgeInsets.only(top: isTablet ? 20 : 10, left: isTablet ? 20 : 0, right: isTablet ? 20 : 0),
      child: Row(
            ...
            child: Column(
              ...
              children: [
                Text(
                  ...
                  style: GoogleFonts.poppins(fontSize: isTablet ? 15 : 13, color: textColor),
                ),
                if (_imageBytes != null) ...[
                  SizedBox(height: 5),
                  Text(
      ...
      return ClipOval(
        child: Image.memory(
          ...
          errorBuilder: (context, error, stackTrace) {
            debugPrint('KYC Screen: Error displaying image from memory: $error');
  ...
  Widget _buildImagePlaceholder(bool isTablet, bool isDarkMode, Color iconColor) {
    return Column(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        Icon(Icons.image_not_supported, size: isTablet ? 40 : 30, color: iconColor),
        SizedBox(height: 5),
        Text(
          'Image\nUnavailable',
  ...
  Future<dynamic> _callApiWithRetry({required Map<String, String> body, int retries = 3}) async {
    for (int i = 0; i < retries; i++) {
      try {
        ...
        if (response.statusCode == 200) {
          ...
          debugPrint('API call failed with status: ${response.statusCode}. Retrying...');
        }
      } catch (e) {
    ...
    try {
      ...
      if (response['data'] is String) {
        try {
          ...
          if (parsedData['result'] != null && parsedData['result']['gstnDetailed'] != null) {
            ...
            if (gstnDetails['additionalPlaceAddress'] != null &&
              ...
              return gstnDetails['additionalPlaceAddress'][0]['address'].toString();
            }
  ...
  Widget _buildVerifiedGstinDetails(bool isTablet, bool isDarkMode) {
    ...
    if (!_isGstinVerified || _gstinDetails == null) return const SizedBox.shrink();

    return Padding(
      padding: EdgeInsets.symmetric(horizontal: isTablet ? 20 : 10, vertical: 10),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(
          ...
          _buildDetailRow("GSTIN Number:", _gstinDetails!['GSTIN'], isTablet, textColor),
          SizedBox(height: isTablet ? 10 : 8),
          _buildDetailRow("Business Address:", _gstinDetails!['Business Address'], isTablet, textColor),
        ],
      ),
    );
  }

  Widget _buildDetailRow(String label, String value, bool isTablet, Color textColor) {
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        SizedBox(
  ...
  Future<Map<String, dynamic>> _submitKycData() async {
    ...
    try {
      ...
      final String kycId = 'KYC${DateTime.now().millisecondsSinceEpoch.toRadixString(16)}';

      // Get the business address from GSTIN details or use a default value
      final String businessAddress = _gstinDetails?['Business Address'] ??
      ...
      for (int attempt = 1; attempt <= 3; attempt++) {
        try {
          ...
          if (_imageBytes != null && _imageBytes!.isNotEmpty) {
            debugPrint('KYC Screen: Adding shop image to multipart request. Size: ${_imageBytes!.lengthInBytes} bytes');

            // Final image size check
            debugPrint('Final image details:');
            debugPrint('- Size: ${_imageBytes!.lengthInBytes} bytes (${(_imageBytes!.lengthInBytes / 1024).toStringAsFixed(1)} KB)');
            ...
            if (_imageBytes!.length > 10) {
              ...
              debugPrint('- Is JPEG: ${_imageBytes![0] == 0xFF && _imageBytes![1] == 0xD8}');
            }
          ...
          debugPrint('KYC Screen: Multipart request files count: ${request.files.length}');

          // Send the request with progressive timeout
          int timeoutSeconds = 60 + (attempt * 30); // 60, 90, 120 seconds
          debugPrint('KYC Screen: Sending request with ${timeoutSeconds}s timeout');

          var streamedResponse = await request.send().timeout(Duration(seconds: timeoutSeconds));
          ...
          if (response.statusCode == 200) {
            ...
            if (decodedResponse['status'] == 'success') {
            ...
            } else if (decodedResponse['status'] == 'error' && decodedResponse['message'] == 'Failed to upload photo') {
              // Photo upload specifically failed, try different approaches
              debugPrint('KYC Screen: Photo upload failed, trying alternative approach on attempt $attempt');
              ...
              if (attempt < 3) {
                ...
                debugPrint('KYC Screen: All photo upload attempts failed, trying without image');
          ...
          if (attempt == 3) {
            ...
            debugPrint('KYC Screen: All multipart attempts failed, trying fallback without image');
            return await _submitKycDataFallback();
          }
  ...
  Future<Map<String, dynamic>> _submitKycDataBase64() async {
    ...
    try {
      ...
      debugPrint('KYC Screen: Base64 image length: ${base64Image.length} characters');
      ...
      debugPrint('KYC Screen: Base64 request body prepared (without base64 data for brevity)');
                  ...
                  child: Form(
                    ...
                    child: Column(
                      children: [
                        ...
                        Image.asset("assets/kyc1.gif", height: isTablet ? 200 : 150),
                        SizedBox(height: isTablet ? 20 : 10),
                        Text('"Safe, Secure, And Hassle-Free KYC"', style: GoogleFonts.poppins(fontSize: isTablet ? 18 : 16, color: textColor), textAlign: TextAlign.center),
                        SizedBox(height: isTablet ? 10 : 5),
                        Text(
                          "Submit Your Details And Unlock Access To All KISANGRO B2B Products",
                          style: GoogleFonts.poppins(fontSize: isTablet ? 16 : 14, color: subtitleColor),
                          textAlign: TextAlign.center,
                        ),
                        Divider(endIndent: isTablet ? 200 : 130, indent: isTablet ? 200 : 130, color: dividerColor),
                        _sectionTitle("Primary Details", isTablet, isDarkMode),
                        _textFormField("Full Name", controller: _fullNameController, isTablet: isTablet, isDarkMode: isDarkMode),
                        _textFormField("Mail Id", controller: _mailIdController, isTablet: isTablet, isDarkMode: isDarkMode),
                        _textFormField("WhatsApp Number", isNumber: true, controller: _whatsAppNumberController, isTablet: isTablet, isDarkMode: isDarkMode),
                        _sectionTitle("Business Details", isTablet, isDarkMode),
                        _textFormField("Business Name", controller: _businessNameController, isTablet: isTablet, isDarkMode: isDarkMode),
                        _textFormField("GSTIN", showVerify: true, controller: _gstinController, isTablet: isTablet, isDarkMode: isDarkMode),
                        _textFormField("Aadhaar Number (Owner)", isNumber: true, controller: _aadhaarNumberController, isTablet: isTablet, isDarkMode: isDarkMode),
                        _textFormField("Business PAN Number", isPAN: true, controller: _panNumberController, isTablet: isTablet, isDarkMode: isDarkMode),
                        _dropdownField("Nature Of Core Business", _natureOfBusinessSelected, (newValue) {
                          setState(() {
                            _natureOfBusinessSelected = newValue;
                          });
                        }, isTablet, isDarkMode),
                        _textFormField("Business Contact Number", isNumber: true, controller: _businessContactNumberController, isTablet: isTablet, isDarkMode: isDarkMode),
                        _buildVerifiedGstinDetails(isTablet, isDarkMode),
                        _sectionTitle("Establishment Photo", isTablet, isDarkMode),
                        _photoUploadBox(isTablet, isDarkMode),
                        SizedBox(height: isTablet ? 30 : 20),
                        Padding(
                          padding: EdgeInsets.symmetric(horizontal: isTablet ? 20 : 8.0),
                          child: Text(
                        ...
                        SizedBox(
                          ...
                          child: ElevatedButton(
                            onPressed: _isSubmitting ? null : () async {
                              debugPrint('KYC Screen: Submit button pressed');
                              FocusScope.of(context).unfocus();
                              ...
                              if (_imageBytes == null || _imageBytes!.isEmpty) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    content: Text('Please upload a shop image before submitting.', style: GoogleFonts.poppins()),
                                    backgroundColor: Colors.red,
                                  ),
                                );
                                return;
                              ...
                              if (_imageBytes!.lengthInBytes > 100000) { // 100KB
                                final shouldContinue = await showDialog<bool>(
                                  context: context,
                                  builder: (context) => AlertDialog(
                                    title: Text('Large Image', style: GoogleFonts.poppins()),
                                    content: Text(
                              ...
                              debugPrint('KYC Screen: All validations passed, submitting data');
                              setState(() {
                                _isSubmitting = true;
                              });
                              ...
                              try {
                                ...
                                if (response['status'] == 'success') {
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content: Text('KYC submitted successfully!', style: GoogleFonts.poppins()),
                                      backgroundColor: Colors.green,
                                    ),
                                  );
                                  ...
                                  debugPrint('KYC Screen: KYC submission failed: ${response['message']}');
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content: Text('KYC submission failed: ${response['message'] ?? 'Unknown error'}', style: GoogleFonts.poppins()),
                                      backgroundColor: Colors.red,
                                    ),
                                  );
                                }
                                ...
                                debugPrint('KYC Screen: Error submitting KYC: $e');

                                // Show more user-friendly error message
                                String errorMessage = 'Error submitting KYC data.';
                            ...
                            child: _isSubmitting
                                ...
                                : Text('Submit KYC', style: GoogleFonts.poppins(fontSize: isTablet ? 18 : 16, color: Colors.white)),
                          ),
                        ),
                        SizedBox(height: isTablet ? 15 : 10),
                        // Skip KYC Button
                        SizedBox(
                          ...
                          child: TextButton(
                            onPressed: _isSubmitting ? null : _skipKycAndProceed,
